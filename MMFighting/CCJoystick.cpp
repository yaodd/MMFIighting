...
void CCJoystick::updateTouchPoint(CCPoint touchPoint)
{
    ...
    angle=atan2(ball->getPositionY()-0, ball->getPositionX()-0)/(3.14159/180);
...
bool CCJoystick::containsTouchLocation(CCTouch* touch)
{
    ...
    if (activeRadius>0)
    {
        if (ccpDistance(touchPoint, getParent()->convertToWorldSpace(ccp(getPositionX(),getPositionY()))) < activeRadius) {
            return true;
        }
    }
